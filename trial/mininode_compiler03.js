...
function getTempVarIndex(ctx) {
  return  ctx['tempVarIdx'];
}

function makeTempVarIndex(ctx) {
...
function generate(tree, lctx) {
  ...
  let varIdx;
  if (tree[0] === 'lit') {
    idx = makeTempVarIndex(lctx);

    return SOFT_TAB + tempVarName(idx) + ' = or i32 ' + tree[1] + ', 0' + BR;
  }
...
function generateBinaryOperator(tree, operator, lctx) {
  ...
  const leftVar = tempVarName(getTempVarIndex(lctx));
  const right = generate(tree[2], lctx);
  const rightVar = tempVarName(getTempVarIndex(lctx));

  let varIdx = makeTempVarIndex(lctx);
  let varName = tempVarName(varIdx);
...
function generateMain(mainBlock, lctx) {
  let varIdx = getTempVarIndex(lctx);
  ...
  block = block + '  ret i32 ' + tempVarName(varIdx) + BR;
  block = block + '}' + BR;
  return block;
}
