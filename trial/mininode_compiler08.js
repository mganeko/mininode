...
function generate(tree, gctx, lctx) {
  ...
  if (tree[0] === 'func_call') {
    ...
    if (func[0] === 'builtin_func') {
      ...
      println('--calling builtin_func name=' + funcName + ', type=' + funcType);
      if (funcType === 'void') {
    ...
    if (func[0] === 'user_defined') {
      const funcName = func[1];
      const funcType = func[2];
      println('--calling user_defined name=' + funcName + ', type=' + funcType);
      /*
      if (funcType === 'void') {
  ...
  // --- literal ---
  if (tree[0] === 'lit') {
    const t = typeof tree[1]; // move typeof to module
    if (t === 'number') {
      // --- int32 literal ---
      const v = makeTempVarName(lctx);
